...
export const handler: Handlers = {
  ...
  async PUT(req, ctx) {
    ...
    try {
      ...
      if (!isEvaluator && !isAdmin) {
        return new Response(JSON.stringify({ error: "No autorizado para actualizar esta evaluación" }), {
          status: 403,
          headers: { "Content-Type": "application/json" },
        });
      }
      ...
      if (evaluation.status === EvaluationStatus.COMPLETED && !isAdmin) {
        return new Response(JSON.stringify({ error: "No se puede modificar una evaluación completada" }), {
          status: 403,
          headers: { "Content-Type": "application/json" },
        });
      }
      ...
      delete body.deliverableId;
      delete body.rubricId;
  ...
  async DELETE(req, ctx) {
    ...
    try {
      ...
      if (!isEvaluator && !isAdmin) {
        return new Response(JSON.stringify({ error: "No autorizado para eliminar esta evaluación" }), {
          status: 403,
          headers: { "Content-Type": "application/json" },
        });
      }
      ...
      if (evaluation.status === EvaluationStatus.COMPLETED && !isAdmin) {
        return new Response(JSON.stringify({ error: "No se puede eliminar una evaluación completada" }), {
          status: 403,
          headers: { "Content-Type": "application/json" },
        });
      }
