...
export const handler = {
  async GET(req: Request, ctx: FreshContext) {
    ...
    try {
      ...
      const teamVelocity = completedStories.length > 0
        ? completedStories.reduce((sum, story) => sum + (story.points || 0), 0) / Math.max(totalSprints, 1)
        : 0;
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
              <div class="flex items-center">
                ...
                <div class="ml-4">
                  ...
                  <p class="text-2xl font-bold text-gray-900">{metrics.completedUserStories}/{metrics.totalUserStories}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
              <div class="flex items-center">
                ...
                <div class="ml-4">
                  ...
                  <p class="text-2xl font-bold text-gray-900">{metrics.activeSprints}/{metrics.totalSprints}</p>
                </div>
              </div>
            </div>
              ...
              <div class="flex items-center">
                <div class="bg-red-100 p-3 rounded-lg">
                  <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 4h6m-6 4h6m-7-7h8"></path>
                  </svg>
                </div>
                <div class="ml-4">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
              <div class="flex items-center">
                ...
                <div class="ml-4">
                  ...
                  <p class="text-2xl font-bold text-gray-900">{metrics.totalTasks > 0 ? Math.round((metrics.completedTasks / metrics.totalTasks) * 100) : 0}%</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {metrics.teamPerformance.length > 0 ? (
                <div class="space-y-4">
                  {metrics.teamPerformance.map((member, index) => (
                    <div key={member.userId} class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div class="flex-1">
                      ...
                      <div class="text-right">
                        <div class="text-sm font-medium text-gray-900">{member.totalTasks} tareas</div>
                ...
                {metrics.sprintSummary.map((sprint) => (
                  <div key={sprint.id} class="border border-gray-200 rounded-lg p-4">
                    ...
                    <div class="flex justify-between text-sm mb-2">
                      <span>Tareas: {sprint.completedTasks}/{sprint.totalTasks}</span>
                      <span>{sprint.totalTasks > 0 ? Math.round((sprint.completedTasks / sprint.totalTasks) * 100) : 0}%</span>
                    </div>
              ...
              <div class="ml-3">
                ...
                <div class="mt-2 text-sm text-blue-700">
                  <p>
                    ...
                    Los datos se actualizan automáticamente conforme el equipo completa tareas y actualiza el estado del proyecto.
                  </p>
                  <p class="mt-1">
                    <strong>Datos mostrados:</strong> {metrics.totalTasks} tareas totales, {metrics.totalUserStories} historias de usuario,
                    {metrics.totalSprints} sprints, {metrics.teamPerformance.length} miembros del equipo.
                  </p>
                </div>
              </div>
            </div>
