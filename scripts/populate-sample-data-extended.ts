...
// import type { Sprint } from "../models/sprint.ts"; // Sprint type might be unused if createdSprints map is not used
...
async function seedDatabase() {
  ...
  const adminUserId = "admin_user_placeholder_id"; // Placeholder, will be replaced by actual admin user's ID
  ...
  try {
    for (const userData of usersToCreate) {
      ...
      if (userData.ref === "admin") {
        // adminUserId = user.id; // This line caused an error, adminUserId is const. Let's ensure createdUsers['admin'] exists.
      }
  ...
  try {
    for (const rubricDef of rubricsData) {
      ...
      const rubricToCreateData: Omit<Rubric, keyof Model | "id" | "createdAt" | "updatedAt" | "criteria"> & { criteria: Omit<RubricCriterion, "id" | "levels"> & { levels: Omit<RubricCriterionLevel, "id">[] }[] } = {
        name: rubricDef.name, description: rubricDef.description, status: rubricDef.status,
        projectId: project?.id,
        criteria: rubricDef.criteria.map(c => ({ ...c, levels: c.levels.map(l => ({...l })) })),
          ...
          userStories: [
            {
              ...
              tasks: [
                { title: "T1.1: Design registration & login UI mockups", status: TaskStatus.DONE, assignedToRef: "dev_charlie", spentHours: 4, description: "Create wireframes and mockups for registration and login pages." },
                { title: "T1.2: Implement registration API endpoint", status: TaskStatus.DONE, assignedToRef: "dev_charlie", spentHours: 6, description: "Develop backend API for user creation." },
                { title: "T1.3: Implement login API endpoint", status: TaskStatus.DONE, assignedToRef: "dev_diana", spentHours: 5, description: "Develop backend API for user authentication." },
                { title: "T1.4: Frontend for registration & login pages", status: TaskStatus.DONE, assignedToRef: "dev_diana", spentHours: 8, description: "Build UI based on mockups." },
                { title: "T1.5: Write unit tests for auth APIs", status: TaskStatus.DONE, assignedToRef: "dev_charlie", spentHours: 4, description: "Ensure API reliability." },
                {
                  refName: "authQaDeliverable", title: "T1.6: Perform QA testing on auth flow", status: TaskStatus.DONE, assignedToRef: "tester_eve", spentHours: 6, description: "Verify complete authentication functionality.",
            ...
            },
            {
              ...
              tasks: [ /* Minimal tasks for brevity */ ]
            },
          ]
        },
        {
          ...
          userStories: [ /* Minimal user stories for brevity */ ]
        },
      ],
      backlogUserStories: [ /* Minimal backlog for brevity */ ]
    },
  ];

  const defaultAdminUserId = createdUsers['admin']?.id || 'unknown_admin_id'; // Fallback, though admin creation is checked
  ...
  for (const projectData of projectsData) {
    ...
    try {
      ...
      const projectPayload = {
        name: projectData.projectName, description: projectData.description,
      ...
      console.log(`Created Project: ${project.name} (ID: ${project.id}, Key: ${project.projectKey})`);

      for (const sprintData of projectData.sprints) {
        console.log(`  Creating Sprint: ${sprintData.sprintName} for project ${project.name}`);
        const sprintCreatorId = createdUsers[sprintData.creatorRef]?.id || scrumMasterId; // Default to SM or Admin
        ...
        for (const usData of sprintData.userStories) {
          ...
          const usCreatorId = createdUsers[usData.creatorRef]?.id || productOwnerId; // Default to PO or Admin
          ...
          if (assignee) {
            updatePayload.assignedTo = assignee.id;
          }
          ...
          for (const taskData of usData.tasks || []) {
            const taskCreatorId = createdUsers[taskData.creatorRef]?.id || scrumMasterId; // Default to SM or Admin
            if (taskData.isDesignatedDeliverable) {
              ...
              const deliverablePayload: Omit<Deliverable, keyof Model | "id" | "createdAt" | "updatedAt" | "attachments" > & { attachments: Attachment[]; isDeliverable: true } = {
                ...
                assignedTo: deliverableAssignedToId, createdBy: taskCreatorId,
                isDeliverable: true,
                submissionInstructions: taskData.submissionInstructions,
                dueDate: taskData.dueDate, submittedAt: taskData.submittedAt,
                submittedBy: deliverableSubmittedById,
                ...
                attachments: (taskData.attachments || []).map(att => {
                  ...
                  return {
                    ...
                    uploadedBy: uploader?.id || defaultAdminUserId,
  ...
  try {
    for (const evalDef of evaluationsData) {
      ...
      const populatedCriteriaEvals = rubric.criteria.map((rubricCriterion) => {
        ...
        return {
          criterionId: rubricCriterion.id || crypto.randomUUID(), // Ensure criterionId is present
          criterionName: rubricCriterion.name, score: currentScore,
          levelDescription: rubricCriterion.levels.find(l => l.pointValue === currentScore)?.description || "N/A", // Match pointValue
      ...
      const evaluationToCreateData: Omit<Evaluation, keyof Model | "id" | "createdAt" | "updatedAt"> = {
        ...
        status: evalDef.status, feedback: evalDef.feedback, criteriaEvaluations: populatedCriteriaEvals,
        maxPossibleScore, totalScore, evaluatedAt: evalDef.evaluatedAtTimestamp || Date.now(),
        projectId: deliverable.projectId || project?.id || "unknown_project_id", // Ensure projectId is present
      };
